...
async function mix_fetch_data(el){
    ...
    if(method == "post" || method == "put"){
        ...
        attrs.forEach(attr => {
            // Disable the input if it is inside a hidden container
            const parentContainer = attr.closest('[style*="display: none"]');
            if (parentContainer) {
                attr.disabled = true;
                return;
            } else {
            ...
            attr.classList.remove("mix-error") 
            const regex = attr.getAttribute("mix-check")
            re = new RegExp(regex)
            cl(re.test(attr.value))
            if( ! re.test(attr.value) ){
                cl("mix-check failed")
                attr.classList.add("mix-error") 
                errors = true
            }
        });
        if(errors) return
